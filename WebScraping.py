...
def Scraping_equipes_NBA(var_url):
    ...
    #On récupére les noms des variables du tableau
    #On récupère donc l'ensemble du texte contenu dans les balises "th" elles-mêmes
    #contenues dans la première balises "tr" (première ligne du tableau : [0])
    headers = [th.getText() for th in divb.findAll('tr', limit=2)[0].findAll('th')]
    
    #On récupére ensuite les lignes du tableau
    #On récupère donc l'ensemble du texte contenu dans les balises "td" elles-mêmes
    #contenues dans des balises "tr" (autres du lignes du tableau :[1:] )
    rows = divb.findAll('tr')[1:]
    rows_data = [[td.getText() for td in rows[i].findAll('td')]
                    for i in range(len(rows))]
    
    #On rajoute un numéro de ligne en 1ère colonne
    numligne = 1
    for i in range(0, len(rows_data)):
        rows_data[i].insert(0, numligne)
        numligne +=1
